  ...
  OIL: [
    {
      ...
      color: "#3b82f6",
      label: "London Open",
      note: "Primary — energy market opens, big displacement candles",
    },
    {
      name: "NY_AM",
      start: 7,
      ...
      color: "#22c55e",
      label: "NY AM",
      note: "EIA/inventory data window, London continuation",
    },
    {
      name: "ASIAN",
      start: 20,
      ...
      color: "#8b5cf6",
      label: "Asian",
      note: "Low liquidity — avoid unless news catalyst",
    },
  ],
  GOLD: [
    {
      ...
      color: "#3b82f6",
      label: "London Open",
      note: "Primary — institutional positioning, DXY correlation",
    },
    {
      name: "NY_AM",
      start: 7,
      ...
      color: "#22c55e",
      label: "NY AM",
      note: "CPI/NFP reaction, continuation setups",
    },
    {
      name: "NY_PM",
      start: 13,
      ...
      color: "#eab308",
      label: "NY PM",
      note: "Low probability — macro catalyst only",
    },
  ],
  NQ: [
    {
      ...
      color: "#22c55e",
      label: "NY Open",
      note: "Primary — first 90min RTH, highest volume",
    },
    {
      name: "NY_PM",
      start: 13,
      ...
      color: "#eab308",
      label: "NY PM",
      note: "Afternoon reversal, VWAP retest setups",
    },
    {
      name: "PRE",
      start: 8,
      ...
      color: "#8b5cf6",
      label: "Pre-Market",
      note: "Futures gap fill — wait for RTH open",
    },
  ],
};
...
function getKZStatus(assetId) {
  const dec = getNYDecimal(),
    zones = ASSET_KILLZONES[assetId] || [];
  for (const z of zones)
    if (dec >= z.start && dec < z.end)
...
async function fetchFearGreed() {
  try {
    ...
    return { value: 50, label: "Neutral", live: false };
  }
}
...
async function fetchReddit() {
  try {
    const r = await fetchJSON(
      "/api/apewisdom/api/v1.0/filter/all-stocks/page/1",
      {},
      6000
    );
    const d = await r.json();
    ...
    const out = {};
    for (const [asset, syms] of Object.entries(map)) {
      const m = d.results.find((x) => syms.includes(x.ticker));
      if (m) {
        const prev = parseInt(m.mentions_24h_ago) || 1,
          curr = parseInt(m.mentions) || 0;
        out[asset] = {
          mentions: curr,
          trend: curr >= prev ? "rising" : "falling",
          delta: Math.round(((curr - prev) / prev) * 100),
          live: true,
        };
      } else {
        out[asset] = { mentions: 0, trend: "neutral", delta: 0, live: false };
      }
    }
    ...
    return {
      data: {
        OIL: { mentions: 0, trend: "neutral", delta: 0, live: false },
        GOLD: { mentions: 0, trend: "neutral", delta: 0, live: false },
        NQ: { mentions: 0, trend: "neutral", delta: 0, live: false },
      },
      live: false,
    };
  }
  ...
  if (FINNHUB_KEY && FINNHUB_KEY !== "your_finnhub_key_here") {
    try {
      ...
      const endpoints = [
        ...
        `https://finnhub.io/api/v1/company-news?symbol=USO&from=${getDateStr(
          -2
        )}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
        `https://finnhub.io/api/v1/company-news?symbol=GLD&from=${getDateStr(
          -2
        )}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
        `https://finnhub.io/api/v1/company-news?symbol=QQQ&from=${getDateStr(
          -2
        )}&to=${getDateStr(0)}&token=${FINNHUB_KEY}`,
      ];
      for (const url of endpoints) {
        try {
          ...
          if (r.ok) {
            const d = await r.json();
            if (Array.isArray(d)) allNews.push(...d);
          }
        } catch {}
      }
      const seen = new Set();
...
async function groq(apiKey, messages, max_tokens = 1800) {
  ...
  if (!r.ok)
    throw new Error(
      `Groq ${r.status}: ${body?.error?.message || "unknown error"}`
    );
...
async function analyzeMarket({ fg, reddit, news, apiKey }) {
  const newsText = news
    .slice(0, 6)
    .map((h, i) => `${i + 1}. ${h.title}`)
    .join("\n");
  const rdText = Object.entries(reddit)
    .map(
      ([k, v]) =>
        `${k}:${v.mentions} mentions ${v.trend}(${v.delta > 0 ? "+" : ""}${
          v.delta
        }%)`
    )
    .join(", ");
...
async function analyzeNews({ news, apiKey }) {
  const newsText = news
    .map(
      (h, i) => `${i + 1}. TITLE: ${h.title}\nDESC: ${h.description || "N/A"}`
    )
    .join("\n\n");
  return groq(
    apiKey,
...
function useMarket(apiKey) {
  ...
  const cachedNews = useRef([]);
  const cachedNewsLive = useRef(false);
  const cachedNewsAt = useRef(null);

  const log = useCallback(
    (msg) =>
      setState((s) => ({
        ...s,
        log: [...s.log.slice(-12), `${getNYTime()} ${msg}`],
      })),
    []
  );

  const refreshNewsOnly = useCallback(async () => {
    log("📡 Fetching live news...");
    try {
      const { news, live, fetchedAt } = await fetchNews();
      cachedNews.current = news;
      cachedNewsLive.current = live;
      cachedNewsAt.current = fetchedAt;
      log(`News: ${news.length} headlines — ${live ? "live" : "fallback"}`);
      setState((s) => ({
        ...s,
        news,
        newsLive: live,
        lastNewsUpdate: fetchedAt,
      }));
      return news;
    } catch (e) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      log(`F&G: ${fg.value} (${fg.label}) — ${fg.live ? "live" : "fallback"}`);

      log("📡 Fetching Reddit sentiment...");
      const { data: reddit, live: redditLive } = await fetchReddit();
      log(`Reddit: ${redditLive ? "live" : "fallback"}`);
      ...
      log(
        `Prices: OIL=${
          prices.OIL?.live
            ? prices.OIL.change + "% (" + prices.OIL.source + ")"
            : "FAILED"
        } | GOLD=${
          prices.GOLD?.live ? prices.GOLD.change + "%" : "FAILED"
        } | NQ=${prices.NQ?.live ? prices.NQ.change + "%" : "FAILED"}`
      );

      log("📡 Fetching live news...");
      const { news, live: newsLive, fetchedAt } = await fetchNews();
      cachedNews.current = news;
      cachedNewsLive.current = newsLive;
      cachedNewsAt.current = fetchedAt;
      log(`News: ${news.length} headlines — ${newsLive ? "live" : "fallback"}`);
      ...
      log("🤖 Running AI market analysis...");
      const market = await analyzeMarket({ fg, reddit, news, apiKey });
      log("✅ Market analysis complete.");

      const conflicts = ASSETS.filter((a) => {
        const aiB = market?.assets?.[a.id]?.bias;
        const priceT = prices?.[a.id]?.trend;
        return (
          aiB &&
          priceT &&
          ((aiB === "Bullish" && priceT === "bearish") ||
            (aiB === "Bearish" && priceT === "bullish"))
        );
      });
      ...
      if (conflicts.length > 0) {
        ...
        const priceContext = ASSETS.map(
          (a) =>
            `${a.id}: live price ${prices[a.id]?.change > 0 ? "+" : ""}${
              prices[a.id]?.change
            }% TODAY (${prices[a.id]?.trend?.toUpperCase()})`
        ).join(", ");
        const marketV2 = await analyzeMarket({
          fg,
          reddit,
          news: [
            {
              title: `LIVE PRICE UPDATE: ${priceContext}. Adjust bias accordingly.`,
            },
            ...news.slice(0, 5),
          ],
          apiKey,
        });
        log("✅ Re-analysis complete.");
        setState((s) => ({
          ...s,
          market: marketV2,
          prices,
...
const ImpactDot = ({ level }) => {
  const c =
    level === "high" ? "#ef4444" : level === "medium" ? "#eab308" : "#64748b";
  return (
    <div
      style={{
        width: 8,
...
function KZBadge({ assetId }) {
  ...
  useEffect(() => {
    const t = setInterval(() => setKz(getKZStatus(assetId)), 20000);
  ...
  return a ? (
    ...
    >
      ...
      >
        {a.label} ACTIVE — {kz.minutesLeft}m
      </span>
    </div>
  ) : (
    <div
    ...
    >
      ...
      >
        Dead Zone — {kz.next?.label} in {kz.minutesUntil}m
      </span>
    </div>
  );
}
...
function KZCard({ asset }) {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setKz(getKZStatus(asset.id));
      setNy(getNYTime());
    }, 20000);
    return () => clearInterval(t);
  }, [asset.id]);
  const a = kz.active,
    zones = ASSET_KILLZONES[asset.id] || [],
    dec = getNYDecimal();
  return (
    <div
      style={{
        ...
        borderRadius: 16,
        padding: 16,
        boxShadow: `0 4px 20px ${asset.glow}`,
      }}
    >
      <div
        style={{
      ...
      {a ? (
        ...
        >
          <div>
            ...
            >
              Next:{" "}
              <span style={{ color: kz.next?.color || "#94a3b8" }}>
                {kz.next?.label}
              </span>
            </div>
          </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `1px solid ${
                  isActive ? z.color + "40" : "rgba(255,255,255,0.05)"
                }`,
                borderRadius: 9,
                padding: "9px 12px",
                opacity: past ? 0.35 : 1,
            ...
            >
              <div
                style={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  background: isActive ? z.color : `${z.color}45`,
                ...
                >
                  <span
                    style={{
                      fontFamily: F,
                      fontSize: 11,
                      color: isActive ? z.color : "rgba(255,255,255,0.38)",
                      letterSpacing: 1,
                    }}
                  >
                    {z.label}
                  ...
                  <span
                    style={{
                      fontFamily: FB,
                      fontSize: 14,
                      color: isActive ? z.color : "rgba(255,255,255,0.22)",
                    }}
                  >
                    {fmtHour(z.start)}–{fmtHour(z.end)}
                  </span>
                </div>
                {isActive && (
                  <div
  ...
  return (
    ...
    >
      <svg width="148" height="80" viewBox="0 0 150 80">
        <path
          d={`M14,74 A${r},${r} 0 0,1 ${cx * 2 - 14},74`}
          fill="none"
        ...
        {["#ef4444", "#f97316", "#eab308", "#22c55e", "#16a34a"].map((c, i) => {
          const sa = ((i * 36 - 90) * Math.PI) / 180,
            ea = ((i * 36 + 33 - 90) * Math.PI) / 180;
          return (
            <path
              key={i}
              d={`M${cx + r * Math.cos(sa)},${
                cy + r * Math.sin(sa)
              } A${r},${r} 0 0,1 ${cx + r * Math.cos(ea)},${
                cy + r * Math.sin(ea)
              }`}
              fill="none"
              stroke={c}
        ...
        <path
          d={`M14,74 A${r},${r} 0 ${pct > 50 ? 1 : 0},1 ${ex},${ey}`}
          fill="none"
          stroke={color}
          strokeWidth="13"
          strokeLinecap="round"
      ...
      <div>
        ...
        <div style={{ fontFamily: FB, fontSize: 52, color, lineHeight: 1 }}>
          {value}
        </div>
        <div
          style={{
...
function DXYStrip({ data }) {
  ...
  const b = data.dxy_bias,
    color =
      b === "Bullish" ? "#ef4444" : b === "Bearish" ? "#22c55e" : "#eab308";
  return (
    <div
      style={{
        background: "rgba(6,8,16,0.97)",
    ...
    >
      ...
      <span style={{ fontFamily: FB, fontSize: 20, color, flexShrink: 0 }}>
        {b === "Bullish" ? "▲" : b === "Bearish" ? "▼" : "◆"} {b.toUpperCase()}
      </span>
      <div
        style={{
          width: 1,
...
// ─── Reddit Strip ──────────────────────────────────────────────────────
function RedditStrip({ reddit, live }) {
  return (
    <div
      style={{
        background: "rgba(6,8,16,0.97)",
    ...
    >
      ...
      {ASSETS.map((a) => {
        const d = reddit[a.id];
        if (!d) return null;
        const c = d.trend === "rising" ? "#22c55e" : "#ef4444";
        return (
          <div
            key={a.id}
          ...
          >
            ...
            <span style={{ fontFamily: F, fontSize: 10, color: c }}>
              {d.trend === "rising" ? "▲" : "▼"}
              {Math.abs(d.delta)}%
            </span>
            <span
...
function AssetCard({ asset, data }) {
  ...
  const bm = BIAS_META[data?.bias] || BIAS_META.Neutral;
  const smtC =
    data?.smt_signal === "Diverging"
      ? "#f97316"
      : data?.smt_signal === "Confirming"
      ? "#22c55e"
      : "#64748b";
  return (
    <div
      style={{
        ...
        borderRadius: 16,
        overflow: "hidden",
        boxShadow: `0 4px 24px ${asset.glow}`,
    ...
    >
      ...
      >
        ...
        {data && (
          ...
          >
            <span style={{ fontFamily: F, fontSize: 12, color: asset.color }}>
              {data.move_type}
            </span>
            <span style={{ fontSize: 18, color: "rgba(255,255,255,0.18)" }}>
              {open ? "▲" : "▼"}
            </span>
          </div>
        )}
      ...
      {open && data && (
        ...
        >
          ...
          >
            {[
              { l: "SENTIMENT", v: data.sentiment_edge },
              { l: "VS CROWD", v: data.crowd_vs_smart },
              { l: "BULL LEVEL", v: data.key_level_bull },
              { l: "BEAR LEVEL", v: data.key_level_bear },
            ].map((f) => {
              const c =
                f.v?.includes("Bull") || f.v?.includes("With")
                  ? "#22c55e"
                  : f.v?.includes("Bear") || f.v?.includes("Against")
                  ? "#ef4444"
                  : "#eab308";
              return (
                <div
          ...
          {data.smt_note && (
            ...
            >
              ...
              >
                SMT — {data.smt_signal?.toUpperCase()}
              </div>
              <div
                style={{
                  fontFamily: F,
          ...
          <div
            style={{
              background: `linear-gradient(135deg,${bm.bg},transparent)`,
              border: `1px solid ${bm.border}`,
              borderRadius: 10,
              padding: "12px 14px",
            }}
...
function NewsTab({ rawNews, newsLive, lastUpdate, apiKey, onRefreshNews }) {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setAnalysisState({ status: "error", data: null, error: e.message });
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        >
          <button
            ...
            style={{
              ...
              borderRadius: 10,
              padding: "12px",
              color: refreshing ? "rgba(255,255,255,0.2)" : "#60a5fa",
              fontFamily: FB,
              fontSize: 16,
              letterSpacing: 2,
          ...
          <button
            onClick={handleScan}
            disabled={analysisState.status === "loading" || !rawNews?.length}
            style={{
              background:
              ...
              letterSpacing: 2,
              cursor:
                analysisState.status === "loading" || !rawNews?.length
                  ? "not-allowed"
                  : "pointer",
              display: "flex",
              alignItems: "center",
      ...
      >
        ...
        >
          📰 LIVE HEADLINES ({rawNews?.length || 0})
        </div>
        {rawNews?.length === 0 && (
          <div
            style={{
          ...
          >
            Tap REFRESH to load latest headlines
          </div>
        )}
        {(rawNews || []).map((h, i) => (
          <div
          ...
          >
            ...
            {h.pubDate && (
              ...
              >
                {new Date(h.pubDate).toLocaleTimeString("en-US", {
                  hour: "2-digit",
                  minute: "2-digit",
                  timeZone: "America/New_York",
                })}{" "}
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {analysisState.data.top_opportunity}
              </div>
            </div>
          </div>
          <div
          ...
          >
            ...
            >
              {analysisState.data.market_summary}
            </div>
          </div>
          <div
            style={{
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {(h.category || "").replace("_", " ").toUpperCase()}
                  </span>
                </div>
                <div
                  style={{
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {(h.assets || []).map((id) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: 11 }}>{asset?.emoji}</span>
                        <span style={{ fontFamily: F, fontSize: 9, color: dc }}>
                          {dir === "bullish"
                            ? "▲"
                            : dir === "bearish"
                            ? "▼"
      ...
      {analysisState.status === "idle" && rawNews?.length > 0 && (
        ...
        >
          ...
          >
            Tap <span style={{ color: "#f97316" }}>AI SCAN</span> to get impact
            level, direction, and trade implication for each headline.
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      {ASSETS.map((a) => {
        ...
        // ── CORE FIX: detect conflict, then use live price as truth ──
        const conflict =
          d?.bias &&
          p?.trend &&
          ((d.bias === "Bullish" && p.trend === "bearish") ||
            (d.bias === "Bearish" && p.trend === "bullish"));

        // If live price contradicts AI, override badge to match live price
        const effectiveBias = conflict
          ? p.trend === "bullish"
            ? "Bullish"
            : "Bearish"
          : d?.bias || null;
        ...
        // Percentages use effectiveBias direction
        const rp =
          ub === "Bullish"
            ? d?.bullish_real_pct ?? 50
            : ub === "Bearish"
            ? d?.bearish_real_pct ?? 50
            : null;
        const tp =
          ub === "Bullish"
            ? d?.bullish_trap_pct ?? 50
            : ub === "Bearish"
            ? d?.bearish_trap_pct ?? 50
            : null;
        ...
        return (
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  ...
                  >
                    ...
                    >
                      <div
                        style={{
                          ...
                          border: `1px solid ${priceColor}35`,
                          borderRadius: 7,
                          padding: "3px 10px",
                        }}
                      >
                    ...
                    {conflict && (
                      ...
                      >
                        ...
                        — badge updated to match. Hit ↻ for full re-analysis.
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* ── AI Bias badge — now uses effectiveBias ── */}
              {effectiveBias && (
                <div
                  style={{
                    marginLeft: "auto",
                    background: ebm.bg,
                    border: `1px solid ${ebm.border}`,
                    borderRadius: 8,
                    padding: "4px 10px",
                    flexShrink: 0,
                  }}
                >
                  <span
                    style={{ fontFamily: F, fontSize: 10, color: ebm.color }}
                  >
                    {ebm.icon} {conflict ? "LIVE" : "AI"}: {effectiveBias}
                    {conflict && (
                      <span
                        style={{ color: "#f97316", marginLeft: 4, fontSize: 9 }}
                      >
                        ↺
                      </span>
                    )}
            ...
            >
              {["Bullish", "Bearish"].map((opt) => {
                const sel = ub === opt;
                const col = opt === "Bullish" ? "#22c55e" : "#ef4444";
                return (
                  <button
                    key={opt}
                    onClick={() =>
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${
                        sel ? col : "rgba(255,255,255,0.08)"
                      }`,
                      borderRadius: 12,
                      padding: 15,
                      color: sel ? col : "rgba(255,255,255,0.28)",
                      fontFamily: F,
                      fontSize: 13,
                      letterSpacing: 2,
                      cursor: "pointer",
            ...
            {/* ── Probability bars when user picks a side ── */}
            {ub && rp !== null && (
              <div
                style={{
                  display: "flex",
              ...
              >
                {[
                  { label: "✅ REAL MOVE", pct: rp, color: "#22c55e" },
                  { label: "⚡ TRAP RISK", pct: tp, color: "#f97316" },
                ].map((b) => (
                  <div key={b.label}>
                    <div
                      style={{
                ...
                <div
                  style={{
                    ...
                    border: `1px solid ${
                      isReal ? "rgba(34,197,94,0.2)" : "rgba(249,115,22,0.2)"
                    }`,
                    borderRadius: 10,
                    padding: "12px 14px",
                    fontFamily: F,
                    fontSize: 12,
                    color: isReal ? "#4ade80" : "#fb923c",
                    lineHeight: 1.7,
                    textAlign: "center",
                  }}
                >
                  {isReal
                    ? `${ub} bias looks REAL — ${rp}% confidence.`
                    : `${ub} looks like a TRAP — ${tp}% trap risk. Wait.`}
                </div>
              </div>
            )}
      ...
      <div style={{ textAlign: "center" }}>
        <div
          style={{
            ...
            letterSpacing: 10,
            background:
              "linear-gradient(130deg,#fff 20%,rgba(255,255,255,0.15))",
      ...
      >
        ...
        >
          ...
          <input
            ...
            onKeyDown={(e) =>
              e.key === "Enter" && key.length > 10 && onSubmit(key)
            }
            style={{
              background: "rgba(255,255,255,0.05)",
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 12,
              padding: 16,
              color: key.length > 10 ? "#fff" : "rgba(255,255,255,0.12)",
              fontFamily: FB,
              fontSize: 22,
              letterSpacing: 4,
              cursor: key.length > 10 ? "pointer" : "not-allowed",
              transition: "all .2s",
            }}
          >
            ENTER THE MARKET
        ...
        >
          ...
          >
            console.groq.com → Sign up → API Keys
            <br />
            <span style={{ color: "#22c55e" }}>
              ✓ Free · Fast · 14,400 calls/day
            </span>
          </div>
        </div>
        <div
        ...
        >
          ...
          >
            Store key in Replit Secrets as
            <br />
            <span style={{ color: "#fb923c" }}>GROQ_API_KEY</span> for
            production use
          </div>
        </div>
      </div>
        ...
        {(log || []).slice(-4).map((l, i, arr) => (
          <div
            ...
            style={{
              ...
              fontSize: 11,
              color:
                i === arr.length - 1 ? "#60a5fa" : "rgba(255,255,255,0.15)",
              letterSpacing: 1,
              textAlign: "center",
            }}
          >
...
export default function App() {
  const [apiKey, setApiKey] = useState(() => {
    try {
      return localStorage.getItem("sf_key") || "";
    } catch {
      return "";
    }
  });
    ...
    >
      ...
      <div
        style={{
          ...
          pointerEvents: "none",
          backgroundImage:
            "linear-gradient(rgba(59,130,246,0.012) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,0.012) 1px,transparent 1px)",
      ...
      <div
        style={{
          ...
          animation: "scanl 10s linear infinite",
          pointerEvents: "none",
          zIndex: 50,
        }}
      />
      ...
      >
        ...
        >
          <div
            style={{
              ...
              letterSpacing: 8,
              background:
                "linear-gradient(130deg,#fff 30%,rgba(255,255,255,0.15))",
              WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent",
              lineHeight: 1,
            }}
          ...
          >
            ICT · OIL · GOLD · NQ · LIVE DATA · GROQ AI
          </div>
        </div>
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Dot
              color={stColor}
              pulse={["live", "fetching", "analyzing"].includes(status)}
              size={7}
            />
            <span
              style={{
          ...
          <div style={{ display: "flex", gap: 7 }}>
            ...
            <button
              ...
              style={{
                ...
                color: isLoading ? "rgba(255,255,255,0.15)" : "#60a5fa",
                fontSize: 16,
                padding: "7px 13px",
                borderRadius: 8,
          ...
          {TABS.map((t) => (
            <button
              ...
              style={{
                flex: 1,
                background:
                  tab === t.id ? "rgba(59,130,246,0.09)" : "transparent",
                border: "none",
                borderBottom:
                  tab === t.id
                    ? "2.5px solid #3b82f6"
                ...
                padding: "13px 2px",
                color: tab === t.id ? "#60a5fa" : "rgba(255,255,255,0.28)",
                fontFamily: F,
                fontSize: 10,
                letterSpacing: 0.5,
                cursor: "pointer",
        ...
        >
          {tab === "intel" && (
            <>
              ...
              <RedditStrip reddit={reddit} live={redditLive} />
              ...
              {market ? (
                <>
                  ...
                  {[
                    {
                      icon: "⚡",
                      label: "PAIR TRADE",
                      text: market.pair_trade,
                      color: "#a855f7",
                    },
                    {
                      icon: "🎯",
                      label: "RISK EVENT",
                      text: market.risk_event,
                      color: "#ef4444",
                    },
                    {
                      icon: "📡",
                      label: "MACRO",
                      text: market.macro_summary,
                      color: "#3b82f6",
                    },
                  ].map((c) => (
                    <div
                  ...
                  {market.correlation_warning &&
                    market.correlation_warning !== "null" && (
                      <div
                        style={{
                          background: "rgba(249,115,22,0.05)",
          ...
          {tab === "kz" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              >
                ...
                >
                  OIL & GOLD → London Open. NQ → NY Open. Each asset has its own
                  optimal trading window.
                </div>
              </div>
              {ASSETS.map((a) => (
                <KZCard key={a.id} asset={a} />
              ))}
            </div>
          )}
        </div>
        ...
        >
          FINNHUB · FEAR & GREED API · APEWISDOM · GROQ LLAMA 3.3 70B
          <br />
